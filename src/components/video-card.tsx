...
export default function VideoCard({ video }: VideoCardProps) {
  ...
  useEffect(() => {
    setIsMounted(true);
    // Safely format date only if video.uploadDate is valid
    try {
    ...
    <Card className="w-80 bg-card rounded-lg shadow-lg overflow-hidden flex flex-col h-full group transition-all duration-300 ease-in-out hover:shadow-2xl">
      <div className="relative aspect-video w-full overflow-hidden">
        <Image
          ...
          priority={false} // Adjust priority based on typical rendering order
        />
      ...
      <CardContent className="p-3 flex-grow flex flex-col">
        ...
        {/* Hover-reveal section for description and more tags */}
