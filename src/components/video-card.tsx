...
export default function VideoCard({ video }: VideoCardProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // In case of an invalid date, still set a string to avoid hydration issues with empty string
  ...
  return (
    <a
      ...
      className="block group" // group class moved here
      aria-label={`Watch video: ${video.title}`}
    >
      <Card className="w-80 bg-card rounded-lg shadow-lg overflow-hidden flex flex-col h-full transition-all duration-300 ease-in-out group-hover:shadow-2xl">
        ...
        <CardContent className="p-3 flex-grow flex flex-col">
          ...
          <div className="opacity-0 max-h-0 group-hover:opacity-100 group-hover:max-h-48 overflow-hidden transition-all duration-500 ease-in-out mt-auto pt-2 group-hover:border-t border-border flex flex-col flex-grow">
            <p className="text-xs text-muted-foreground mb-2 line-clamp-4 overflow-y-auto flex-grow" title={video.description || 'No description available.'}>
            ...
            {video.tags && video.tags.length > 0 && (
              <div className="flex items-start text-xs text-muted-foreground mt-1 pt-1 border-t border-border/50">
                ...
                <div className="flex flex-wrap gap-1">
                  {video.tags.slice(0, 5).map(tag => ( // Limit tags shown on hover for brevity
